...
-   **PACF** : La PACF coupe après le premier lag, montrant une corrélation significative seulement pour le premier lag, qui est négative. Les autres lags décroissent vers zero.
...
autocovariance <- function(X, lag) {
  n <- length(X)
  mean_X <- mean(X)
  cov <- sum((X[1:(n - lag)] - mean_X) * (X[(1 + lag):n] - mean_X)) / (n - lag)
  return(cov)
}

# Calcul des autocovariances jusqu'à lag 20
autocovariances <- sapply(0:20, function(lag) autocovariance(Xt, lag))

# Tracé des autocovariances
plot(0:20, autocovariances, type = "h", main = "Autocovariance", xlab = "Lag", ylab = "Autocovariance")
```

L’observation de la fonction d’autocorrélation pour votre simulation correspond-elle au comportement théorique ?
